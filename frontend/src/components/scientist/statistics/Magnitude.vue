      ...
      <div v-for="(group, gIndex) in sequences" :key="gIndex">
        ...
        <table class="table bordered hovered">
          <thead>
            <tr>
              <th>Image</th>
              <th v-for="(cat, m) in group.picture_set.pictures[0].magnitudes" :key="m" class="overflow-wrap">
                {{ cat.magnitude }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(picture, c) in group.picture_set.pictures" :key="c">
              ...
              <td v-for="(magnitude, cIndex) in picture.magnitudes" :key="cIndex">
                {{ magnitude.result.length }}
              </td>
            </tr>
          </tbody>
        </table>
...
//       this.$axios.post(`/result-magnitude-estimations/${this.$route.params.id}/statistics`, {
...
//         let magnitu = Array.from({ length: ticks }, (v, k) => {
//           return { magnitude: this.minValue + k }
//         })

//         sequences.forEach(sequence => {
//           sequence.picture_set.pictures.forEach(image => {
//             image.magnitudes = magnitu
//             image.magnitudes.forEach(item => {
//               answers.forEach(answer => {
//                 if (answer.magnitude === item.magnitude && answer.picture_id === image.id) {
...
//           sequence.picture_set.pictures.forEach(image => {
//             let row = []
//             image.magnitudes.forEach(magn => {
//               // categ.forEach(result => {
//               row.push(magn.result.length)
//               // })
//             })
//             set.push(row)
//           })
