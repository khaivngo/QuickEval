...
if($res[0][0] == "Superuser")	{				//fetches all users
	...
	try {
		$stmt = $db->prepare("Select person.email from person JOIN workplacebelongs ON person.id = workplacebelongs.personId WHERE workplacebelongs.workPlace = '" . $workPlace . "' AND email != 'NULL' ");
