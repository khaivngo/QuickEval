...
# is the user logged in and has a permission level of more than 2
...
if ($option == "newExperiment") {
    try {
        ...
        $sql = "INSERT INTO
            `experiment` (
                ...
                `isPublic`, `allowColourBlind`, `allowArtifactMarking`, `backgroundColour`,
    ...
    $allowArtifactMarking = $_GET['allowArtifactMarking'];
    $hidden = $_GET['hidden'];
    $timer = $_GET['timer'];
    ...
    $sth->bindParam(3, $allowArtifactMarking);
    $sth->bindParam(4, $backColour);
    $sth->bindParam(5, $forcedPick);
    $sth->bindParam(6, $showOriginal);
    $sth->bindParam(7, $samePairTwice);
    $sth->bindParam(8, $timer);
    $sth->bindParam(9, $experimentId);
    $sth->execute();
