...
function toggleSelect($images) {
    $('#delete-image-set').fadeToggle();    //Displays/hides delete button

    if ($images) {                        //Displays "set original" button if managing images and not sets
        $('#image-set-original').fadeToggle();
        $('#image-rename').fadeToggle();
        $('#picture-name-div').fadeToggle();
    }
    ...
    if ($('#toggle-select').hasClass('start')) {            //If user presses start selecting
        ...
        $(document.body).on('click', '.tile', function() {  //Turns on new listener hvis allows selecting
...
function displayImageSet($imageSet) {
    ...
    $('#right-panel').load('ajax/scientist/viewImageSet.html', function() { //Loads imput fields, buttons
        ...
        setUpButtons(true);                         
        $('#delete-image-set').hide();          //Hides buttons/input fields for deleting/original/renaming
        $('#image-set-original').hide();
        $('#image-rename').hide();
        $('#picture-name-div').hide();
    });
}
